...
def learning(data):
    ...
    for i in range(len(data)):
        ...
        y = df.iloc[:, 2].values  
        #first 10 days are for train (minute wise sampling = > 1440 records/day)
        X_train = X[slice(0,14400)]
        y_train = y[slice(0,14400)]
        #day 11 and day 12 are for test
        X_test = X[slice(14400,17280)]
        y_test = y[slice(14400,17280)]
        ...
        #model evaluation:
        acc_score_rf = round(metrics.accuracy_score(y_test, y_pred),2)
        acc_score_RF.append(acc_score_rf)
        #RMSE
        errRF = round(sqrt(mean_squared_error(y_test, y_pred)),2)
        ...
        yy = df.iloc[:, 2].values
        #first 10 days are for train (minute wise sampling = > 1440 records/day)
        XX_train = XX[slice(0,14400)]
        yy_train = yy[slice(0,14400)]
        #day 11 and day 12 are for test
        XX_test = XX[slice(14400,17280)]
        yy_test = yy[slice(14400,17280)]
        ...
        #model evaluation:
        acc_score_knn = round(metrics.accuracy_score(yy_test, yy_pred),2)
        acc_score_KNN.append(acc_score_knn)
        #RMSE
        errKNN = round(sqrt(mean_squared_error(yy_test, yy_pred)),2)
        ...
        plt.legend()
        plt.savefig('./img/RF_vs_K-NN_sliding_dataset1_for_'+'{0}.png'.format(data[i].split(".")[0]), bbox_inches='tight')
        plt.show()
        plt.close('all')
