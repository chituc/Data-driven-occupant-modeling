...
def learning(data):
    ...
    for i in range(len(data)):
        ...
        y = df.iloc[:, 2].values  
        #days 2-11 are for train (minute wise sampling = > 1440 records/day)
        X_train = X[slice(1440,15840)]
        y_train = y[slice(1440,15840)]
        #days 12-13 are for test
        X_test = X[slice(15840,18720)]
        y_test = y[slice(15840,18720)]
        ...
        yy = df.iloc[:, 2].values
        #days 2-11 are for train (minute wise sampling = > 1440 records/day)
        XX_train = XX[slice(1440,15840)]
        yy_train = yy[slice(1440,15840)]
        #days 12-13 are for test
        XX_test = XX[slice(15840,18720)]
        yy_test = yy[slice(15840,18720)]
        ...
        plt.legend()
        plt.savefig('./img/RF_vs_K-NN_sliding_dataset2_for_'+'{0}.png'.format(data[i].split(".")[0]), bbox_inches='tight')
        plt.show()
        plt.close('all')
