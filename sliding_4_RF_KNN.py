...
def learning(data):
    ...
    for i in range(len(data)):
        ...
        y = df.iloc[:, 2].values  
        #days 4-13 are for train (minute wise sampling = > 1440 records/day)
        X_train = X[slice(4320,18720)]
        #X_train.shape
        y_train = y[slice(4320,18720)]
        #y_train.shape
        #days 14-15 are for test
        X_test = X[slice(18720,21600)]
        #X_test.shape
        y_test = y[slice(18720,21600)]
        #y_test.shape
        ...
        yy = df.iloc[:, 2].values
        #days 4-13 are for train (minute wise sampling = > 1440 records/day)
        XX_train = XX[slice(4320,18720)]
        yy_train = yy[slice(4320,18720)]
        #days 14-15 are for test
        XX_test = XX[slice(18720,21600)]
        yy_test = yy[slice(18720,21600)]
        ...
        t = np.arange(2879)
        plt.plot(t,y_pred[:2879], label='predicted values with RF',color='r',linestyle='dashed')
        plt.plot(t,y_test[:2879], label='test values',color='k', linewidth = 1.5)
        plt.plot(t,yy_pred[:2879], label='predicted values with K-NN',color='g', linestyle='dashed')
        #plt.title('Predicted values versus test data values for Room 1 using RF')
        plt.xlabel('Time steps')
        plt.ylabel('Number of occupants')
        plt.legend()
        plt.savefig('./img/RF_vs_K-NN_sliding_dataset4_for_'+'{0}.png'.format(data[i].split(".")[0]), bbox_inches='tight')
        plt.show()
        plt.close('all')
